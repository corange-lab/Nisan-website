...
function olt_urls(array $CFG){
  return [
    ...
    'STATS' => $CFG['BASE'].'/action/onustatistics.html',
  ];
}
...
function olt_login_or_reuse(array $CFG){
  ...
  $cookie = $_SESSION['olt_cookie_stats'] ?? null;
  ...
  $cookie = tempnam(sys_get_temp_dir(), "oltc_stats_");
  $ch = curl_init();
  curl_setopt_array($ch, [
    CURLOPT_RETURNTRANSFER=>true, CURLOPT_FOLLOWLOCATION=>true,
    CURLOPT_CONNECTTIMEOUT=>$CFG['TIMEOUT'], CURLOPT_TIMEOUT=>$CFG['TIMEOUT'],
    ...
    CURLOPT_HTTPHEADER=>[
      "Origin: {$CFG['BASE']}",
      "Referer: {$CFG['BASE']}/action/login.html",
  ...
  $_SESSION['olt_cookie_stats'] = $cookie;
